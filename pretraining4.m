function [layers, numlayers] = pretraining4(X, Y, numhids, batchsize, maxepoch, continuous)

if nargin <3
    numhid=500; numpen=500; numpen2=2000; numtop = 100;
    
else
    numlayers = numel(numhids);
    numhid = numhids(1);
    numpen = numhids(2);
    numpen2 = numhids(3);
    numtop = numhids(4);
...
restart=1;
rbm;
hidtop=vishid; toprecbiases2=hidbiases; pengenbiases2=visbiases;
save mnistptclassify hidtop toprecbiases2 pengenbiases2;
...
function [data, labels] = cell2double2(X,Y)
nums = 0;
for i =1: length(X)
